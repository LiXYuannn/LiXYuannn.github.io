...
{% include image.html src="assets/images/posts/1-1.png" alt="blog配图" %}
根据论文图表⬇️
{% include image.html src="assets/images/posts/1-2.png" alt="blog配图" %}
 现有假图像检测数据集存在局限，如UADFV规模小，ForgeryNet仅关注人脸、早期通用数据集依赖GAN且数据有限。
 -->GenImage数据集构建的必要性

## 数据集构建
 
{% include image.html src="assets/images/posts/1-3.png" alt="blog配图" %}
 包含超100万对真假图像，使用ImageNet所有真实图像，依1000个标签生成1350000张假图像
 
{% include image.html src="assets/images/posts/1-4.png" alt="blog配图" %}
用8种生成模型生成假图像，每个模型为每类生成近相同数量图像，保证数据集平衡，输入句子依ImageNet标签，部分模型输入语言有调整

{% include image.html src="assets/images/posts/1-5.png" alt="blog配图" %}
...
{% include image.html src="assets/images/posts/1-6.png" alt="blog配图" %}
检测器能轻松识别同一生成器生成的假图像，说明生成器会留下高度一致的痕迹（如特定频率模式、纹理特征等）。而我们需要提升检测器的泛化能力，即独立于所使用的生成器来区分图像真伪的能力。 
 →跨生成器图像分类
### 单模型 跨生成器图像分类
{% include image.html src="assets/images/posts/1-7.png" alt="blog配图" %}
先在SD V1.4上用七种不同的方法训练的模型 然后用八种不同的检测器进行检验
//该表格可反应模型在特定训练数据下的泛化能力，根据各检测器检测准确率平均之前数据可知，**Swin-T的泛化能力最强**

### 多模型 全组合测试
{% include image.html src="assets/images/posts/1-8.png" alt="blog配图" %}
...
{% include image.html src="assets/images/posts/1-9.png" alt="blog配图" %}
检测器应该对这些挑战具有鲁棒性

→通过评估检测器在这些退化图像上的性能，使之更准确的模拟实际条件
{% include image.html src="assets/images/posts/1-10.png" alt="blog配图" %}
...
## 数据分析
{% include image.html src="assets/images/posts/1-11.png" alt="blog配图" %}
...
{% include image.html src="assets/images/posts/1-12.png" alt="blog配图" %}

* 控制变量分析可得到，数据集标签的数量对准确度的影响程度远大于数据数量的影响程度
* 假图像检测器的泛化能力高度依赖训练数据的类别覆盖度，其中100类以上可达到较好效果
{% include image.html src="assets/images/posts/1-13.png" alt="blog配图" %}
...
{% include image.html src="assets/images/posts/1-14.png" alt="blog配图" %}

### 泛化性能优异：
{% include image.html src="assets/images/posts/1-15.png" alt="blog配图" %}
